...
[[nodiscard]] inline std::string to_base64(std::string_view data) {
    ...
    const auto s = [&data](size_t index) { return static_cast<uint8_t>(data[index]); };
    std::string ret;
    const ssize_t origLen = data.size();
...
inline size_t from_base64(const void* src, size_t srcLen, void* dst, size_t dstLen) {
    const std::string_view data { static_cast<const char*>(src), srcLen };
    ...
    for (size_t i = 0; i < origLen; i += 4) {
        out[i / 4 * 3 + 0] = base64_lut[static_cast<uint8_t>(data[i+0])][0] | base64_lut[static_cast<uint8_t>(data[i+1])][1];
        out[i / 4 * 3 + 1] = base64_lut[static_cast<uint8_t>(data[i+1])][2] | base64_lut[static_cast<uint8_t>(data[i+2])][3];
        out[i / 4 * 3 + 2] = base64_lut[static_cast<uint8_t>(data[i+2])][4] | base64_lut[static_cast<uint8_t>(data[i+3])][5];
    }
