...
int base64_test(void)
{
   ...
   const unsigned char special_case[] = { 0xbe, 0xe8, 0x92, 0x3c, 0xa2, 0x25, 0xf0, 0xf8, 0x91, 0xe4, 0xef,
                                          0xab, 0x0b, 0x8c, 0xfd, 0xff, 0x14, 0xd0, 0x29, 0x9d, 0x00 };
   ...
   for (x = 0; x < sizeof(url_cases) / sizeof(url_cases[0]); ++x) {
      ...
      if (url_cases[x].flag == strict) {
         ...
         DO(base64url_strict_decode(url_cases[x].s, slen1, tmp, &l1) == CRYPT_INVALID_PACKET ? CRYPT_OK
         ...
         DO(base64url_strict_decode(url_cases[x].s, slen1, tmp, &l1) == CRYPT_INVALID_PACKET ? CRYPT_OK
                                                                                             : CRYPT_FAIL_TESTVECTOR);
         DO(base64url_sane_decode(url_cases[x].s, slen1, tmp, &l1) == CRYPT_INVALID_PACKET ? CRYPT_OK
                                                                                           : CRYPT_FAIL_TESTVECTOR);
         DO(base64url_decode(url_cases[x].s, slen1, tmp, &l1));
         ...
         DO(base64url_sane_decode(url_cases[x].s, slen1, tmp, &l1) == CRYPT_INVALID_PACKET ? CRYPT_OK
                                                                                           : CRYPT_FAIL_TESTVECTOR);
         DO(base64url_decode(url_cases[x].s, slen1, tmp, &l1) == CRYPT_INVALID_PACKET ? CRYPT_OK
                                                                                      : CRYPT_FAIL_TESTVECTOR);
      }
      l2 = sizeof(out);
      if (x == 0) {
